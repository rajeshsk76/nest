...
function setHeadlineTodo(section: OrgNode, todo: TodoKeyword): void {
  if (!('children' in section) || !Array.isArray(section.children)) return
  const headline = section.children.find((c) => c.type === 'headline') as Headline | undefined
  if (!headline) return
...
export function updateTitleInSource(
  ...
  const tree = parseOrg(source)
  const section = findSectionByPath(tree, path)
  if (!section || !('children' in section) || !Array.isArray(section.children)) {
    return source
  }
  const headline = section.children.find((c) => c.type === 'headline') as Headline | undefined
...
export function collectTodayItems(
  ...
  for (const file of files) {
    for (const h of listHeadlines(file.source, file.id)) {
      ...
      if (h.todo === 'TODO' && reasons.length === 0) {
        // Open TODOs without a date still appear in Today as open work.
        reasons.push('todo')
  ...
  const weight = { deadline: 0, scheduled: 1, todo: 2 }
  return items.sort((a, b) => weight[a.reason] - weight[b.reason] || a.title.localeCompare(b.title))
}
